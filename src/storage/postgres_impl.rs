...
impl<T: Send + Sync + Clone + 'static> PostgresStorage<T> {
    ...
    pub async fn new(db_config: DatabaseConfig, name: impl Into<String>) -> Result<Self> {
        let name = name.into();
        let provider =
            Arc::new(PostgresProvider::new(db_config, format!("{}_provider", &name)).await?);
...
pub trait PostgresStorageExt<T: Send + Sync + Clone + 'static> {
    ...
    fn store_in_transaction<F, Fut, R>(
        &self,
        data: T,
        store_fn: F,
    ) -> impl std::future::Future<Output = Result<R>> + Send
    ...
    fn retrieve<F, Fut, R, Q: Send + Sync + Clone + 'static>(
        &self,
        query: Q,
        retrieve_fn: F,
    ) -> impl std::future::Future<Output = Result<R>> + Send
    ...
    fn delete<F, Fut, R, Q: Send + Sync + Clone + 'static>(
        &self,
        query: Q,
        delete_fn: F,
    ) -> impl std::future::Future<Output = Result<R>> + Send
...
pub trait PostgresBatchExt<T: Send + Sync + Clone + 'static> {
    ...
    where
        F: Fn(&mut deadpool_postgres::Transaction<'_>, Vec<T>) -> Fut
            + Send
            + Sync
            + Clone
            + 'static,
